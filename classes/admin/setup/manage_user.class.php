...
//				}
				...
				$valData = serialize($valData);
			} elseif($keyData == 'user_fullname'){
				$valData = $this->getFullName($empid);
			}
	...
	function getTableList() {
		...
		$sql = "SELECT au.*,aud.ud_name, CONCAT('$viewLink','$editLink','$delLink') as viewdata,
				...
				CONCAT(pi.pi_fname,' ',COALESCE(CONCAT(pi.pi_mname,' '),''),pi.pi_lname) as fullname
					FROM app_users au
					JOIN app_userdept aud on (au.ud_id = aud.ud_id)
					JOIN emp_masterfile em on (em.emp_id=au.emp_id)
					JOIN emp_personal_info pi on (pi.pi_id=em.pi_id)
					$criteria
					$strOrderBy";
	...
	function getActiveEmployeeList(){
		$sql = "select emp_id, CONCAT(b.pi_fname,' ',COALESCE(CONCAT(b.pi_mname,' '),''),b.pi_lname) as fullname 
	...
	function getFullName($emp_id_ = null){
		$sql = "select CONCAT(b.pi_fname,' ',COALESCE(CONCAT(b.pi_mname,' '),''),b.pi_lname) as fullname 
				from emp_masterfile a
				join emp_personal_info b ON b.pi_id=a.pi_id
				WHERE emp_id=?";
