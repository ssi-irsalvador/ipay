		...
		for ($rowPos; $rowPos <= $rowCnt; $rowPos++) {
			...
			if ( ($psa_status != NULL) || ($psa_type != NULL) || ($psa_clsfication != NULL) || ($psa_procode != NULL) || ($psa_name != NULL) || ($psa_tax != NULL) || ($psa_statutory != NULL) ) {
				...
				if ($psa_procode == NULL) {
					...
					if ( ($psa_procode != 'Bonus Code') && ($psa_procode != 'Compensation Code') && ($psa_procode != 'Non Taxable Code') && ($psa_procode != 'Recurring Tax Projection Code') && ($psa_procode != 'De Minimis Code') && ($psa_procode != 'Reimbursement Code')) {
						$_SESSION['eMsg'][] = "In Row {$rowPos}, Column \"Property Code\" should be one of these only: 'Bonus Code', 'Compensation Code', 'Non Taxable Code', 'Recurring Tax Projection Code' or 'De Minimis Code' or 'Reimbursement Code'.";
						$isValid = false;
					}
				}
				if ($psa_name == NULL) {
