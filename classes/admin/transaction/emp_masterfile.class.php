...
//			$rsResult->fields['pi_hdmf'] = explode('-',$rsResult->fields['pi_hdmf']);
	...
	function dbFetchLeave($id_ = "") {
		$sql = "select * from emp_leave 
				join leave_type on leave_type.leave_id=emp_leave.leave_id
				where empleave_id=?";
		$rsResult = $this->conn->Execute($sql,array($id_));
		if (!$rsResult->EOF) {
			return $rsResult->fields;
...
//		exit;
		...
		// SqlAll Query
		$sql = "SELECT DISTINCT am.*,CONCAT('$emp_num_') as emp_idnum, CONCAT('$emp_lname_') as pi_lname, comp.comp_name, CONCAT('$emp_fname_') as pi_fname, IF(pinfo.pi_mname!='', CONCAT(UPPER(SUBSTRING(pinfo.pi_mname,1,1)),'.'),'') as pi_mname, post.post_name, pinfo.pi_emailone,
				CONCAT('$viewLink','$empinfoLink') as viewdata, dept.ud_name, bran.branchinfo_name
						...
						LEFT JOIN payroll_pps_user ppuser on (ppuser.emp_id=am.emp_id)
		...
		$arrFields = array(
		 "viewdata"=>"<a href=\"?statpos=emp_masterfile&action=add\"><img src=\"".SYSCONFIG_DEFAULT_IMAGES_INCTEMP."icons/edited/add.png\" title=\"Add New\" border=0 width=\"16\" height=\"16\"></a>"
		,"emp_idnum"=>"Emp No"
		,"pi_lname"=>"Last Name"
		,"pi_fname"=>"First Name"
		,"pi_mname"=>"MI"
...
//		$this->conn->debug=1;
		...
		// Add Option for Image Links or Inline Form eg: Checkbox, Textbox, etc...
		$editLink = "<a href=\"?statpos=emp_masterfile&empinfo=',sinfo.emp_id,'&empsalaryinfoedit=',sinfo.salaryinfo_id,'#tab-2\"><img src=\"".SYSCONFIG_DEFAULT_IMAGES_INCTEMP."icons/edited/edit.png\" title=\"Edit\" hspace=\"2px\" border=0 width=\"16\" height=\"16\"></a>";
		...
		// SqlAll Query
		$sql = "SELECT CONCAT('$editLink','$delLink') as viewdata, am.*, stype.*, sinfo.*, pinfo.pi_lname, comp.comp_name, pinfo.pi_fname, pinfo.pi_emailone,
...
//		exit;		
	...
	function saveOTInfo($id_= ""){
		...
		if (!$rsResult_->EOF) {
			...
			$this->conn->Execute($sqlupdatepps);
		} else {
			$flds_[]="pps_id='".$_POST['pps_id']."'";
			$flds_[]="emp_id='".$id_."'";
