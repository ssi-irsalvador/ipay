...
class clsBankExportMetrobank{
	 ...
	 */
	function clsBankExportMetrobank($dbconn_ = null){
		$this->conn =& $dbconn_;
		$this->fieldMap = array(
...
//       printa($gData);exit;
	...
	function generateReport($gData = array(),$pData = array()){
        $qry = array();
        $qry[] = "h.payperiod_id = ".$gData['payperiod_id']."";
        $qry[] = "c.emp_stat in ('1','7','10')";
        $qry[] = "g.banklist_id = ".$gData['banklist_id']."";
        $criteria = (count($qry)>0)?" WHERE ".implode(" and ",$qry):"";
        $sql = "SELECT g.bank_routing_number, g.bank_acct_name, g.bank_company_code, CONCAT(e.pi_lname,', ',e.pi_fname) as name,f.bankiemp_acct_name as bname,f.bankiemp_acct_no,c.emp_id
					...
					JOIN bank_info g on (g.banklist_id=f.banklist_id)
					JOIN bank_list i on (i.banklist_id=g.banklist_id)
                $criteria
                order by e.pi_lname";
        $rsResult = $this->conn->Execute($sql);
        $varNum = 1;
		while(!$rsResult->EOF){
	        ...
	        $text_hash = 2;
	        $text_hash .= str_pad($rsResult->fields['bank_routing_number'],3,'0',STR_PAD_LEFT);
	        $text_hash .= 26;
	        $text_hash .= '001';
	        $text_hash .= str_pad($pData['bank_routing_number'],3,'0',STR_PAD_LEFT);
	        $text_hash .= '0000000';
	        $text_hash .= str_pad($rsResult->fields['bank_acct_name'],40,' ',STR_PAD_RIGHT);
	        //employee name
	        $text_hash .= trim($rsResult->fields['bankiemp_acct_no']);
	        //Transaction Code fixed, User Code fixed and DRCR Amount(net pay)
	        $text_hash .= str_pad(str_replace('.', "",number_format($arrData[$rsResult->fields['emp_id']]['net'],2,'.','')),15,'0',STR_PAD_LEFT);
	        $text_hash .= 9;
	        $text_hash .= str_pad($rsResult->fields['bank_company_code'],5,'0',STR_PAD_LEFT);
			$date = new DateTime($gData['pbr_credit_date']);
			$result = $date->format('mdY');
	        $text_hash .= $result;
	        //FILLER
...
//        echo $net."<br>";
     ...
     * @author IR Salvador
     */
    function generateLetter($gData = array(),$bankInfo= array(), $pData = array()){
    	//printa($gData); exit;
    	$orientation='P';
        $unit='mm';
        $format='LETTER';
        ...
        $arrData = $this->generateReport($gData, $pData);
//      echo count($arrData);
//		printa($arrData);
//      printa($_POST);
//      printa($gData);exit;
        ...
        foreach($arrData as $key => $val){
            ...
            $oPDF->MultiCell(200, 2, $bankInfo['comp_name'],0,'L',1);
            ...
            $amount .= "PhP ".number_format($net,2,".",",");
        ...
        foreach($arrData as $key => $val ){
            ...
            if($row_ctr  > $maxPerPage || $emp_ctr >= count($arrData)){
                ...
                if($emp_ctr >= count($arrData)){
            		...
            		$oPDF->SetXY($coordX, $coordY+35);
            		$oPDF->MultiCell(200, 10, "Prepared By: ",0,'L',1);
            		$oPDF->SetFont('helvetica', 'b', '10');
            		$oPDF->SetXY($coordX, $coordY+50);
            		...
            		$oPDF->SetFont('helvetica', '', '10');
            		$oPDF->SetXY($coordX+110, $coordY+35);
            		$oPDF->MultiCell(200, 10, "Approved By: ",0,'L',1);
            		$oPDF->SetFont('helvetica', 'b', '10');
            		$oPDF->SetXY($coordX+110, $coordY+50);
            		$oPDF->MultiCell(200, 10, strtoupper($pData['pbr_approved_by']),0,'L',1);
            		$oPDF->SetXY($coordX+110, $coordY+55);
            		$oPDF->MultiCell(200, 10, $pData['pbr_approved_pos'],0,'L',1);
                }
        ...
        */
        ...
        //$hashFooter_ .= str_pad(number_format($net,0,'.',''),13,'0',STR_PAD_LEFT);
        // total hash
        //$hashFooter_ = str_pad($ctr,6,'0',STR_PAD_LEFT);
        //$hashFooter_ .= str_pad(str_replace('.', "", $net),15,'0',STR_PAD_LEFT);
        $hashFooter_ = "";
        // get the pdf output
        $output = $oPDF->Output("payroll_transaction_prooflist".date('Y-m-d').".pdf","S");
        if(!empty($output)){
